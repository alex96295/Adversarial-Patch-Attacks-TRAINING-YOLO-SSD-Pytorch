...
if __name__ == '__main__':
    ...
    class PatchTrainer(object):
        ...
        def __init__(self, mode):
            ...
            if use_cuda:
                self.darknet_model_yolov4 = self.darknet_model_yolov4.eval().to(self.device)  # TODO: Why eval? test!
                self.patch_applier = PatchApplier().to(self.device)
                self.patch_transformer = PatchTransformer().to(self.device)
                self.score_extractor_yolo = yolov4_feature_output_manage(0, 80, self.config).to(self.device)
                ...
                self.darknet_model_yolov4 = self.darknet_model_yolov4.eval()  # TODO: Why eval?
                self.patch_applier = PatchApplier()
                self.patch_transformer = PatchTransformer()
                self.score_extractor_yolo = yolov4_feature_output_manage(0, 80, self.config)
        ...
        def train(self):
            ...
            img_size = 608  # 608 for this yolov4 cfg
            
            destination_path = "./"
            destination_name = 'loss_tracking_yv4_obj_cls.txt'
            destination_name2 = 'loss_tracking_compact_batch_yv4_obj_cls.txt'
            destination_name3 = 'loss_tracking_compatc_epochs_yv4_obj_cls.txt'
            ...
            for epoch in range(n_epochs):
                ...
                for i_batch, (img_batch, lab_batch) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}',
                    ...
                    with autograd.detect_anomaly():
                        ...
                        im = transforms.ToPILImage('RGB')(p_img_batch.cpu()[0])
                        # plt.imshow(im)
                        # plt.show()
                      
                        loss_type = "max_approach"

                        output = self.darknet_model_yolov4(p_img_batch)  
                        score_yolov4 = self.score_extractor_yolo(output, loss_type)
                        ...
                        det_loss = torch.mean(score_yolov4)
                        ...
                        if i_batch + 1 >= len(train_loader):
                            ...
                            del adv_batch_t, output, score_yolov4, det_loss, p_img_batch, nps_loss, tv_loss, loss
                ...
                if True:
                    ...
                    # plt.imshow(im)
                    # plt.show()
                    im.save("/scratch/msc20f3/master_thesis/saved_patches_mytrial/ale_yv4_obj_cls_1000ep.jpg")
